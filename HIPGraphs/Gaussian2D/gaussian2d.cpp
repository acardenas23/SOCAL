...
#define HIP_CHECK(error)                                                                           \
  {                                                                                                \
    ...
    if ((localError != hipSuccess) && (localError != hipErrorPeerAccessAlreadyEnabled)) {          \
      std::cout << "Error: " << hipGetErrorString(localError) << "\n    Code: " << localError              \
                     << "\n    Str: " << #error << "\n    In File: " << __FILE__                   \
                     << "\n    At line: " << __LINE__;                                            \
    }                                                                                              \
  }
...
void Gaussian2DGPU(){
    ...
    Gaussian2DGraph(m_hip, a_hip, b_hip, t);
...
void Gaussian2DGraph(float *m_hip, float *a_hip, float *b_hip, int t){
    ...
    hipStream_t stream;
    hipGraph_t graph {nullptr}; //structure and content of graph
    hipGraphExec_t graph_exec {nullptr}; //executable graph
  ...
  //  {
      ...
      //  printf("\nNum of nodes in the graph = %zu\n", numNodes);
        HIP_CHECK(hipGraphInstantiate(&graph_exec, graph, NULL, NULL, 0));
 //       graphCreated = true;
  //  }
...
void InitPerRun() {
    ...
    for (i = 0; i < Size * Size; i++)
        *(m + i) = 0.0;
        printf("Initialized contents of multiplier matrix m");
}
